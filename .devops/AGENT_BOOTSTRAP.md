...
1. **Repository context**
   - Project lives at `/Volumes/DatenAP/Code/vod.arkturian.com`.
...
4. **Deployment target**
   - Built files go to `/var/www/vod.arkturian.com`.
   - Backups are stored with prefix `vod-server` under `/var/backups/` (configurable via templates).

5. **Startup checklist for new session**
   - `cd /Volumes/DatenAP/Code/vod.arkturian.com`.
